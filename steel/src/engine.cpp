...
void Engine::Init(SharedPtr<GameInfo> game_info)
{
...
void Engine::SetActors(SharedPtr<ActorManager> actors)
{
    this->actors = actors;
    this->renderer = CreateSharedPtr<Renderer>(actors);
}
	...
	{	
		...
		{   //LOCKED FRAMERATE, NO INTERPOLATION
			while (frame_accumulator >= desired_frametime * update_multiplicity) 
			{
				...
				{
                    //STEEL_INFO("SDL_Delay: {0}", fixed_deltatime);
					//No need for `SDL_Delay(???)` here, if you're curious just 
                    // compute the framerate inside `ActorManager::Update`
                    this->actors->Update(fixed_deltatime);
